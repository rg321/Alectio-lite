...
def get_loaders():
	...
	mytrainloader = torch.utils.data.DataLoader(train_dataset, batch_size=BATCH_SIZE,
	                                          shuffle=True, num_workers=2 ,collate_fn=generate_batch)

	mytestloader = torch.utils.data.DataLoader(test_dataset, batch_size=BATCH_SIZE,
	                                         shuffle=False, num_workers=2 ,collate_fn=generate_batch)
...
if __name__ == '__main__':
	...
	#print(list(infer_outs.keys())[:20])

	#print(len(infer_outs.keys()))
